...
21. **Logically synchronous architecture** -- The architecture is fully synchronous in design. Disk I/O uses `node:fs` sync methods. LLM API calls (`callModel`) and tool execution (`executeTool`) use `async`/`await` only because TypeScript has no synchronous network I/O -- this is a platform concession, not an architectural choice. The constraint: every `await` must be immediately awaited with no work happening behind it. No `Promise.all`, no concurrent execution, no fire-and-forget. The code reads as a sequential procedure that happens to have `async`/`await` annotations on two call sites. The Rust version will be fully synchronous (`ureq` + `std::fs` + `std::process::Command`) with no async runtime. Discovered during Phase 1 implementation; the original roadmap assumed async I/O throughout.

22. **Agent as headless worker process, orchestrator deferred** -- The agent process is a headless worker: it receives structured messages on stdin, processes them through `processTurn()`, and emits structured messages on stdout. stdin/stdout is the transport; a JSON-lines protocol determines expressiveness. This is not rigid -- it's matched to the architecture. The places where pipes would feel constraining (multiplexed mid-turn commands, graceful mid-turn cancellation, real-time state observation) are exactly the places the architecture intentionally doesn't go: one agent per process, logically synchronous, no concurrent work. Alternative transports (Unix sockets, HTTP) would add complexity for capabilities the architecture doesn't use. A separate orchestrator process (async, potentially Python or TypeScript) will own agent processes and handle out-of-band concerns: spawning/stopping agents, routing external events, managing multiple agents. The orchestrator is deferred -- the agent is complete without it. When built, the orchestrator reads agent state from the conversation store on disk (not through the agent's pipes), making observation a non-issue.
